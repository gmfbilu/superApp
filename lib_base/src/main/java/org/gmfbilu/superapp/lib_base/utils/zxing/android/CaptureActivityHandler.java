...
package org.gmfbilu.superapp.lib_base.utils.zxing.android;
...
import org.gmfbilu.superapp.lib_base.utils.zxing.camera.CameraManager;
import org.gmfbilu.superapp.lib_base.utils.zxing.decode.DecodeThread;
import org.gmfbilu.superapp.lib_base.utils.zxing.view.ViewfinderResultPointCallback;
...
public final class CaptureActivityHandler extends Handler {
    ...
    public void handleMessage(Message message) {
        if (message.what == R.id.zxing_restart_preview) {// 重新预览
            restartPreviewAndDecode();
        } else if (message.what == R.id.zxing_decode_succeeded) {// 解码成功
            state = State.SUCCESS;
        ...
        } else if (message.what == R.id.zxing_decode_failed) {// We're decoding as fast as possible, so when one decode fails,
            // start another.
            // 尽可能快的解码，以便可以在解码失败时，开始另一次解码
            state = State.PREVIEW;
            cameraManager.requestPreviewFrame(decodeThread.getHandler(),
                    R.id.zxing_decode);
        } else if (message.what == R.id.zxing_return_scan_result) {//扫描结果，返回CaptureActivity处理
