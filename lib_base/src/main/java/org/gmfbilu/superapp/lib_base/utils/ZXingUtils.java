...
public class ZXingUtils {
    ...
    public static Bitmap createQRImage(String url, final int width, final int height) {
        try {
            ...
            int[] pixels = new int[width * height]; // 下面这里按照二维码的算法，逐个生成二维码的图片， // 两个for循环是图片横列扫描的结果
    ...
    protected static Bitmap encodeAsBitmap(String contents, BarcodeFormat format,
        ...
        int[] pixels = new int[width * height]; // All are 0, or black, by default
        for (int y = 0; y < height; y++) {
            int offset = y * width;
            for (int x = 0; x < width; x++) {
     ...
     * @return
     */
    protected static Bitmap creatCodeBitmap(String contents, int width, int height, Context
            context) {
     ...
     * @return
     */
    protected static Bitmap mixtureBitmap(Bitmap first, Bitmap second, PointF
            fromPoint) {
